...
public class TimepickController {
    ...
    public String save(@PathVariable String key, @RequestBody PickInfo pickInfo, @RequestBody List<PickDetail> pickDetail) {

        // 트랜잭션 삽입 ?
        try {
            Timetable table = timetableService.findByKey(key);
            pickInfo.setTableId(table.getTableId());
            
            timepickService.savePickInfo(pickInfo); // build?
